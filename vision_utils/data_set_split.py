...
parser.add_argument('--data_set', default='/media/milton/research/dataset/with_logo', type=str,help='Dataset Path, each class must in its own dir')
parser.add_argument('--output',default='/media/milton/research/dataset/split_dataset', type=str,help='Out save dir')
...
parser.add_argument('--test_enable', default=False, type=bool, help='will make test set or not')
...
def train_val_split(args):
    ...
    val_amount=5
    print("Val amount:{}".format(val_amount))
    for i in tqdm(range(len(all_paths))):
        path = all_paths[i]
        class_name = path.split('/')[-2]
        class_name=class_name.strip().replace(' ','_')
        if i%val_amount==0:
            class_dir = os.path.join(val_dir)
            if not os.path.isdir(class_dir):
                os.makedirs(class_dir)
            shutil.copy(path, os.path.join(class_dir,os.path.basename(path)))
        elif i%val_amount==1 and test_enable==True:
            class_dir = os.path.join(test_dir)
            if not os.path.isdir(class_dir):
                os.makedirs(class_dir)
            shutil.copy(path, os.path.join(class_dir,os.path.basename(path)))
        else:
            class_dir = os.path.join(train_dir)
            if not os.path.isdir(class_dir):
                os.makedirs(class_dir)
            shutil.copy(path, os.path.join(class_dir,os.path.basename(path)))
